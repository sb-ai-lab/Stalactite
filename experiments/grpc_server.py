...
from prometheus_async.aio import time as prometheus_time
import torch

from generated_code import services_pb2, services_pb2_grpc
from utils import (
    aggregate_tensors_list,
    batch_generator,
    save_data,
    load_data,
    PingResponse,
...
class CommunicationManager:
    ...
    async def aggregate_clients_results(
            self,
            request: services_pb2.SafetensorDataProto | services_pb2.TensorProto,
            serialization: Serialization,
    ) -> services_pb2.SafetensorDataProto | services_pb2.TensorProto:
        """
        Process client data, return aggregated clients response.
        """
        data = load_data(request, serialization=serialization)  # TODO add numpy / tensor loading
        ...
        return await self._aggregate_unary_results(client_iteration, client_id, serialization)

    async def _aggregate_unary_results(
            self, iteration: int, client_id: str, serialization: Serialization
    ...
    async def aggregate_clients_batched_results(
            self,
            request_iterator: AsyncIterator[services_pb2.SafetensorDataProto] | AsyncIterator[services_pb2.TensorProto],
        ...
        """
        read = asyncio.create_task(self._collect_batched_requests(request_iterator, serialization=serialization))
        client_id, client_iteration = await read
        write = asyncio.create_task(
            self._send_batched_response(context, client_id, client_iteration, serialization=serialization)
        )
        await write

    async def _collect_batched_requests(
            self,
            request_iterator: AsyncIterator[services_pb2.SafetensorDataProto],
            serialization: Serialization,
    ):
        """
        ...
        async for request in request_iterator:
            request_data = load_data(request, serialization=serialization)
            client_id = request.client_id
            client_iteration = request.iteration
    ...
    async def _send_batched_response(
            self, context: grpc.aio.ServicerContext, client_id: str, iteration: int, serialization: Serialization
    ):
        """
        ...
        async with self._multi_multi_condition:
            await self._multi_multi_condition.wait_for(
                lambda: (len(self._agg_results_batched[iteration]) == self.world_size) \
                        and (self._ready_clients == self.world_size)
            )
...
class GRpcCommunicatorServicer(services_pb2_grpc.CommunicatorServicer):
    ...
    async def ExchangeNumpyDataUnaryUnary(
        ...
        Exchanging serialization: protobuf
        """
    ...
    async def ExchangeNumpyDataStreamStream(
        ...
        Exchanging serialization: safetensors
        """
        await self.comm_manager.aggregate_clients_batched_results(
    ...
    async def ExchangeBinarizedDataStreamStream(
        ...
        """
        await self.comm_manager.aggregate_clients_batched_results(
            request_iterator, context, serialization=Serialization.safetensors
        )
