    ...
    async def DecodeMessage(
        ...
        try:
            ...
            encrypted_data = deserialization_func(
                self.tenseal_secret_context(master_id=request.master_id), request.encrypted_data
            )
            data = encrypted_data.decrypt()
            if len(data_shape) == 1:
                return arbiter_pb2.DataMessage(
                    master_id=request.master_id, decrypted_tensor=save_data(torch.tensor(data))
                )
            else:
